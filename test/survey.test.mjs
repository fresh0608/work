...
try {
  ...
  assert.equal(config.features.some((feature) => feature.questions.some((question) => question.text.includes('有没有'))), false);
  assert.equal(config.features.some((feature) => feature.questions.some((question) => question.text.includes('会不会找不到'))), false);
